...
func TestGetGethProcessJobTopicListByPagination(t *testing.T) {
	...
	mock.ExpectQuery("^SELECT (.+) FROM geth_process_job_topics").WillReturnRows(mockRows)
	foundChains, err := GetGethProcessJobTopicListByPagination(mock, &_start, &_end, _order, _sort, filters)
	if err != nil {
	...
	for i, foundChain := range foundChains {
		if cmp.Equal(foundChain, testChains[i]) == false {
			t.Errorf("Expected Chain From Method GetGethProcessJobTopicListByPagination: %v is different from actual %v", foundChain, testChains[i])
		}
	}
	if err = mock.ExpectationsWereMet(); err != nil {
		t.Errorf("There awere unfulfilled expectations: %s", err)
...
func TestGetGethProcessJobTopicListByPaginationForErr(t *testing.T) {
	...
	if len(foundChains) != 0 {
		t.Errorf("Expected From Method GetGethProcessJobTopicListByPagination: to be empty but got this: %v", foundChains)
	}
	if err = mock.ExpectationsWereMet(); err != nil {
		t.Errorf("There awere unfulfilled expectations: %s", err)
	}
