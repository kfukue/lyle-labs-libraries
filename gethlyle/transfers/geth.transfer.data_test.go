...
func TestGetTotalTransfersCount(t *testing.T) {
	...
	numOfChains, err := GetTotalTransfersCount(mock)
	if err != nil {
		t.Fatalf("an error '%s' in GetTotalTransfersCount", err)
	}
	if *numOfChains != numOfChainsExpected {
		t.Errorf("Expected Chain From Method GetTotalTransfersCount: %d is different from actual %d", numOfChainsExpected, *numOfChains)
	}
	if err = mock.ExpectationsWereMet(); err != nil {
		t.Errorf("There awere unfulfilled expectations: %s", err)
	}
...
func TestGetTotalMinersForErr(t *testing.T) {
	...
	numOfChains, err := GetTotalTransfersCount(mock)
	if err == nil {
		t.Fatalf("expected an error '%s' in GetTotalTransfersCount", err)
	}
	if numOfChains != nil {
		t.Errorf("Expected numOfChains From Method GetTotalTransfersCount to be empty but got this: %v", numOfChains)
	}
	if err = mock.ExpectationsWereMet(); err != nil {
		t.Errorf("There awere unfulfilled expectations: %s", err)
	}
